...
def merkle_damgard_aes128(msg: bytes, H: bytes, state_size: int) -> bytes:
    if len(H) != state_size:
    ...
    for i in range(0, len(msg), AES.block_size):
        # pad H to key size
        assert len(H) == state_size
        H += bytes(AES.block_size - len(H))

        # encrypt
        msg_block = msg[i:i + AES.block_size]
        H = AES.new(H, AES.MODE_ECB).encrypt(msg_block)
        H = H[:state_size]

    return H


def generate_collisions(n: int):
    pass
